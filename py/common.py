...
def clean_string(token):
    t = token.lower()
    return t.replace(' ', '_').replace('\'', '').replace('-', '_').replace('+', '_or_more').replace(',', '').replace(')', '').replace('(', '').replace('.', '').replace('/', '_').replace('*', '').replace('<br>', '_').replace('%', '_percent')
...
def write_footer(outFile, headerFile):
    ...
    outFile.write(f'#endif // __ADNDTK__SRC__{includeGuard}\n')
...
def write_heading(outFile, headerFile, version):
    ...
    includeGuard = headerName.replace('.', '_').upper()
    outFile.write(f'#ifndef __ADNDTK__SRC__{includeGuard}\n')
    outFile.write(f'#define __ADNDTK__SRC__{includeGuard}\n\n')
